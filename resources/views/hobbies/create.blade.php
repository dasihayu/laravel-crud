...
@section('body')
    <h1 class="text-2xl font-medium">Add Hobbies</h1>
    <hr>
    
    <form class="max-w-sm mx-auto flex flex-col gap-2 mt-4" action="{{ route('hobbies.store') }}" method="POST">
        @csrf
        <div>
            <label for="title class="block text-sm font-medium text-gray-900">Hobbies</label>
            <div class="relative">
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5" placeholder="Lari dari kenyataan">
            </div>
        </div>
